...
from .tokens import *
from .utils import EMPTY_SET, ListIterator, StringBuilder, Set
...
[/#if]
        ...
        self.trace_enabled = [#if grammar.debugParser]True[#else]False[/#if]
[#if grammar.treeBuildingEnabled]
        self.build_tree = [#if grammar.treeBuildingDefault]True[#else]False[/#if]
        self.tokens_are_nodes = [#if grammar.tokensAreNodes]True[#else]False[/#if]
        self.unparsed_tokens_are_nodes = [#if grammar.unparsedTokensAreNodes]True[#else]False[/#if]
...
[#if grammar.faultTolerant]
        ...
        self.debug_fault_tolerant = [#if grammar.debugFaultTolerant]True[#else]False[/#if]
...
[/#if]
        ...
        while result is None:
            self._next_token_type = None
            next = self.token_source.get_next_token()
            previous = next
[#list grammar.parserTokenHooks as methodName]
            next = ${methodName}(next)
[/#list]
            if not next.unparsed:
                result = next
        ...
        if self.currently_parsed_production is not None:
            ...
            for prod in prods:
                if self.currently_parsed_production == prod:
                    return True
        raise NotImplementedError('stack search not implemented')
        return False
